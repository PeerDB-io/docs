...
1.  **Run the following commands in your terminal:**
    ...
    # Run docker containers: peerdb-server, postgres as catalog, temporal. It might take a few minutes
...
Run the following command to kick off Change Data Capture (CDC) based streaming from source peer to target peer for the `test` table.
    ...
    CREATE MIRROR cdc_mirror FROM source TO target WITH TABLE MAPPING (public.test:public.test);
    ```
...
Run the following command to kick off query based streaming from source peer to target peer. You are able to transform data on the source before streaming them to the target
    ```sql
    --- Query based mirrors require the destination table to be created beforehand, which we did in one of the previous steps.
    CREATE MIRROR qrep_mirror FROM source TO target
    FOR $$
        SELECT id, c1+c2 AS c12, right(t, 7) AS st FROM test WHERE id >= {{.start}} AND id < {{.end}}
    $$ WITH (
