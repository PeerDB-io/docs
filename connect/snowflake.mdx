...
<Info>
  To get the `account_id`, do the following steps:

  `SELECT CURRENT_ACCOUNT();` in Snowflake will give you the account identifier. The host domain of the snowflake instance (must include the account, region, cloud environment).
  Example: `<account_identifier>.us-east-2.aws.snowflakecomputing.com`. In the `account_id` field, you only need to `<account_identifier>.<region>.<cloud>` part,
  essentially all the way up to the `.snowflakecomputing.com` part, excluding the `.snowflakecomputing.com` part.
</Info>
