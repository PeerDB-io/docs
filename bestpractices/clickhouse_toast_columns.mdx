...
Refer to [this blog post](https://xata.io/blog/replica-identity-full-performance) for performance considerations when setting `REPLICA IDENTITY FULL`. **TL;DR** the impact from REPLICA IDENTITY FULL will likely be manageable as long as the replicated tables have a primary key!
