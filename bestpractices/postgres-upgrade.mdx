...
Set the `last_offset` field of the mirrors (whose source peer you are upgrading) to 0 in the `metadata_last_sync_state` table in the `catalog` Postgres container.
You can `psql` into that container and run :
```sql
UPDATE metadata_last_sync_state SET last_offset = 0 WHERE job_name = <mirror_name_of_this_peer>;
```
