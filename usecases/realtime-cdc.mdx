...
PeerDB introduces **CREATE MIRROR for streaming changes** to enable real-time Change Data Capture (CDC) from PostgreSQL to the Target peer. Currently we support **PostgreSQL (running anywhere)**, **Azure Eventhub**, **Kafka**, **BigQuery** and **Snowflake** as the target Peers.
