...
WITH (
  ...
  snapshot_sync_mode='<default_or_avro>',
...
2. **snapshot_sync_mode**: `default` uses csv format and `avro` converts data to avro format before syncing to target. Both use the COPY command under the covers.
